        ...
        public async Task<PartialViewResult> GetAllContractsByDate_statusAsync(string _max, string _mini, string status,string id)
        {
            ...
            if (!string.IsNullOrEmpty(_max) && !string.IsNullOrEmpty(_mini) && _max.Length > 0)
            {
                ...
                else
                {
                    ...
                    if (status == "deported")
                    {
                        FinancialTransactionOfEmployeeAll = FinancialTransactionOfEmployeeAll.Where(x => x.CrCasAccountReceiptIsPassing =="3"|| x.CrCasAccountReceiptIsPassing == "4").OrderBy(x=>x.CrCasAccountReceiptDate).ToList();
                    }
                    else if (status == "Reserved")
                    {
                        FinancialTransactionOfEmployeeAll = FinancialTransactionOfEmployeeAll.Where(x => x.CrCasAccountReceiptIsPassing == "2").OrderBy(x => x.CrCasAccountReceiptDate).ToList();
                    }
                    else if (status == "Custody")
                    {
                        FinancialTransactionOfEmployeeAll = FinancialTransactionOfEmployeeAll.Where(x => x.CrCasAccountReceiptIsPassing == "1").OrderBy(x => x.CrCasAccountReceiptDate).ToList();
                    }
                    else
                    {
