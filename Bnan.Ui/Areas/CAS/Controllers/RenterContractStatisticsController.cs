...
namespace Bnan.Ui.Areas.CAS.Controllers
{
    ...
    public class RenterContractStatisticsController : BaseController
    {
        ...
        public async Task<IActionResult> GetAllByType(string Type ,string listDrop ,string singleNo ,string startDate,string endDate)
        {
            ...
            if (Type == "Age")
            {
                var AllRenters2_Age = _unitOfWork.CrCasRenterContractStatistic.FindAll(x => currentCar.CrMasUserInformationLessor == x.CrCasRenterContractStatisticsLessor).DistinctBy(x => x.CrCasRenterContractStatisticsAgeNo).OrderBy(x=>x.CrCasRenterContractStatisticsAgeNo).ToList();
