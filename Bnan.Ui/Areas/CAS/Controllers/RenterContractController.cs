...
namespace Bnan.Ui.Areas.CAS.Controllers
{
    ...
    public class RenterContractController : BaseController
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var RenterContractAll = _unitOfWork.CrCasRenterLessor.FindAll(x => x.CrCasRenterLessorCode == currentUser.CrMasUserInformationLessor && x.CrCasRenterLessorContractCount > 0, new[] { "CrCasRenterLessorNavigation", "CrCasRenterLessorMembershipNavigation" }).OrderByDescending(x => x.CrCasRenterLessorDateLastContractual).ToList();
        ...
        public async Task<PartialViewResult> GetRenterContractByStatusAsync(string status)
        {
            ...
            if (!string.IsNullOrEmpty(status))
            {
                ...
                if (status == Status.All)
                {
                    ...
                    var RenterContractbyStatusAll = _unitOfWork.CrCasRenterLessor.FindAll(l => l.CrCasRenterLessorCode == currentUser.CrMasUserInformationLessor && l.CrCasRenterLessorContractCount > 0, new[] { "CrCasRenterLessorNavigation", "CrCasRenterLessorMembershipNavigation", "CrCasRenterContractStatistics" }).OrderByDescending(x => x.CrCasRenterLessorDateLastContractual).ToList();
                    return PartialView("_DataTableRenterContract", RenterContractbyStatusAll);
                }
                var RenterContractbyStatus = _unitOfWork.CrCasRenterLessor.FindAll(x => x.CrCasRenterLessorStatus == status && x.CrCasRenterLessorCode == currentUser.CrMasUserInformationLessor && x.CrCasRenterLessorContractCount > 0, new[] { "CrCasRenterLessorNavigation", "CrCasRenterLessorMembershipNavigation", "CrCasRenterContractStatistics" }).OrderByDescending(x => x.CrCasRenterLessorDateLastContractual).ToList();
        ...
        public async Task<IActionResult> Edit(string id)
        {
            ...
            var contract = _unitOfWork.CrCasRenterLessor.Find(x => x.CrCasRenterLessorId == id, new[] { "CrCasRenterLessorNavigation", "CrCasRenterLessorMembershipNavigation", "CrCasRenterContractStatistics" });
            if (contract == null)
            {
            ...
            //var RenterContract_Invoices_All = _unitOfWork.CrCasAccountInvoice.FindAll(x => currentUser.CrMasUserInformationLessor == x.CrCasAccountInvoiceLessorCode && x.CrCasAccountInvoiceBranchCode == currentUser.CrMasUserInformationDefaultBranch).ToList();
            var RenterContract_Invoices_All = _unitOfWork.CrCasAccountInvoice.FindAll(x => currentUser.CrMasUserInformationLessor == x.CrCasAccountInvoiceLessorCode).ToList();

            RenterContractEditVM renterContractEditVM = new RenterContractEditVM();
            renterContractEditVM.CasRenterContractVM = model;
            renterContractEditVM.CrMasSysEvaluation = Mechanism;
            renterContractEditVM.CrCasRenterContractBasic = RenterContract_Basic_All;
            renterContractEditVM.CrCasAccountInvoice = RenterContract_Invoices_All;

            return View(renterContractEditVM);
        }
        ...
        public async Task<IActionResult> FailedMessageReport_NoData()
        {
            ...
            ViewBag.id = "#sidebarRenter";
            ViewBag.no = "1";
            var titles = await setTitle("205", "2203002", "2");
            await ViewData.SetPageTitleAsync(titles[0], titles[1], titles[2], "", "", titles[3]);
            var (mainTask, subTask, system, currentUser) = await SetTrace("203", "2203002", "2");
