    ...
    public class CarStatisticsController : BaseController
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var Most_Frequance_Company_list = _unitOfWork.CrCasCarInformation.GetAll()
                                    .GroupBy(q => q.CrCasCarInformationLessor)
                                    .OrderByDescending(gp => gp.Count());

            var Most_Frequance_Company = Most_Frequance_Company_list.FirstOrDefault()?.Key?.ToString();

            var AllCars = _unitOfWork.CrCasCarInformation.FindAll(x => Most_Frequance_Company == x.CrCasCarInformationLessor, new[] { "CrCasCarInformation1" }).ToList();
            ...
            var Branch_count = _unitOfWork.CrCasCarInformation.FindAll(x => Most_Frequance_Company == x.CrCasCarInformationLessor && x.CrCasCarInformationBranchStatus != Status.Deleted).DistinctBy(x => x.CrCasCarInformationBranch).Count();
            var City_count = Branch_count;
            var rigon_count = Branch_count;
            var Brand_count = _unitOfWork.CrCasCarInformation.FindAll(x => Most_Frequance_Company == x.CrCasCarInformationLessor).DistinctBy(x => x.CrCasCarInformationBrand).Count();
            var year_count = _unitOfWork.CrCasCarInformation.FindAll(x => Most_Frequance_Company == x.CrCasCarInformationLessor).DistinctBy(x => x.CrCasCarInformationYear).Count();
            var Category_count = _unitOfWork.CrCasCarInformation.FindAll(x => Most_Frequance_Company == x.CrCasCarInformationLessor).DistinctBy(x => x.CrCasCarInformationCategory).Count();
            var Model_count = _unitOfWork.CrCasCarInformation.FindAll(x => Most_Frequance_Company == x.CrCasCarInformationLessor).DistinctBy(x => x.CrCasCarInformationModel).Count();
            var Status_count = _unitOfWork.CrCasCarInformation.FindAll(x => Most_Frequance_Company == x.CrCasCarInformationLessor).DistinctBy(x => x.CrCasCarInformationStatus).Count();
        ...
        public async Task<IActionResult> GetAllByType(string Type, string listDrop, string singleNo , string company_now)
        {
            ...
            var AllCars = _unitOfWork.CrCasCarInformation.FindAll(x => company == x.CrCasCarInformationLessor, new[] { "CrCasCarInformation1" }).ToList();
            ...
            {
                var Branch_count = _unitOfWork.CrCasCarInformation.FindAll(x => company == x.CrCasCarInformationLessor && x.CrCasCarInformationBranchStatus != Status.Deleted).DistinctBy(x => x.CrCasCarInformationBranch).Count();
                var City_count = Branch_count;
                var rigon_count = Branch_count;
                var Brand_count = _unitOfWork.CrCasCarInformation.FindAll(x => company == x.CrCasCarInformationLessor).DistinctBy(x => x.CrCasCarInformationBrand).Count();
                var year_count = _unitOfWork.CrCasCarInformation.FindAll(x => company == x.CrCasCarInformationLessor).DistinctBy(x => x.CrCasCarInformationYear).Count();
                var Category_count = _unitOfWork.CrCasCarInformation.FindAll(x => company == x.CrCasCarInformationLessor).DistinctBy(x => x.CrCasCarInformationCategory).Count();
                var Model_count = _unitOfWork.CrCasCarInformation.FindAll(x => company == x.CrCasCarInformationLessor).DistinctBy(x => x.CrCasCarInformationModel).Count();
                var Status_count = _unitOfWork.CrCasCarInformation.FindAll(x => company == x.CrCasCarInformationLessor).DistinctBy(x => x.CrCasCarInformationStatus).Count();
            ...
            if (Type == "Branches")
            {
                var AllBranches = _unitOfWork.CrCasBranchInformation.FindAll(x => company == x.CrCasBranchInformationLessor && x.CrCasBranchInformationStatus != Status.Deleted).ToList();
            ...
            if (Type == "Brand")
            {
                var AllCars2_Brand = _unitOfWork.CrCasCarInformation.FindAll(x => company == x.CrCasCarInformationLessor, new[] { "CrCasCarInformationBrandNavigation" }).Where(x => x.CrCasCarInformationBrandNavigation?.CrMasSupCarBrandStatus != Status.Deleted).DistinctBy(x => x.CrCasCarInformationBrand).ToList();
            ...
            if (Type == "year")
            {
                var AllCars2_year = _unitOfWork.CrCasCarInformation.FindAll(x => company == x.CrCasCarInformationLessor && x.CrCasCarInformationStatus != Status.Deleted).DistinctBy(x => x.CrCasCarInformationYear).ToList();
            ...
            if (Type == "Model")
            {
                var AllCars2_Model = _unitOfWork.CrCasCarInformation.FindAll(x => company == x.CrCasCarInformationLessor, new[] { "CrCasCarInformationModelNavigation" }).Where(x => x.CrCasCarInformationModelNavigation?.CrMasSupCarModelStatus != Status.Deleted).DistinctBy(x => x.CrCasCarInformationModel).ToList();
            ...
            if (Type == "City")
            {
                var AllCars2_City = _unitOfWork.CrCasCarInformation.FindAll(x => company == x.CrCasCarInformationLessor, new[] { "CrCasCarInformationCityNavigation" }).Where(x => x.CrCasCarInformationCityNavigation?.CrMasSupPostCityStatus != Status.Deleted).DistinctBy(x => x.CrCasCarInformationCity).ToList();
            ...
            if (Type == "Rigon")
            {
                var AllCars2_Rigon = _unitOfWork.CrCasCarInformation.FindAll(x => company == x.CrCasCarInformationLessor, new[] { "CrCasCarInformationRegionNavigation" }).Where(x => x.CrCasCarInformationRegionNavigation?.CrMasSupPostRegionsStatus != Status.Deleted).DistinctBy(x => x.CrCasCarInformationRegion).ToList();
