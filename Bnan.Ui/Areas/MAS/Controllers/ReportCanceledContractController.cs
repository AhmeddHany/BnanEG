...
namespace Bnan.Ui.Areas.MAS.Controllers
{
    ...
    [Area("MAS")]
    [Authorize(Roles = "MAS")]
    public class ReportCanceledContractController : BaseController
    {
        ...
        private readonly IStringLocalizer<ReportCanceledContractController> _localizer;


        public ReportCanceledContractController(UserManager<CrMasUserInformation> userManager, IUnitOfWork unitOfWork,
            IMapper mapper, IUserService userService, ICompnayContract compnayContract,
            IUserLoginsService userLoginsService, IToastNotification toastNotification, IWebHostEnvironment webHostEnvironment, IStringLocalizer<ReportCanceledContractController> localizer) : base(userManager, unitOfWork, mapper)
        {
        ...
        public async Task<PartialViewResult> GetAllContractsByDate_status(string _max, string _mini)
        {
            ...
            if (!string.IsNullOrEmpty(_max) && !string.IsNullOrEmpty(_mini))
            {
                ...
                reportActiveContractVM.All_Lessors = AllLessor;
                return PartialView("_DataTableReportCanceledContract", reportActiveContractVM);
