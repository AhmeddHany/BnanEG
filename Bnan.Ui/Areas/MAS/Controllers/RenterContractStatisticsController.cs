...
namespace Bnan.Ui.Areas.MAS.Controllers
{
    ...
    public class RenterContractStatisticsController : BaseController
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var Nationality_count = _unitOfWork.CrCasRenterContractStatistic.FindAll(x => company == x.CrCasRenterContractStatisticsLessor).DistinctBy(x => x.CrCasRenterContractStatisticsNationalities).Count();
            var MemperShip_count = _unitOfWork.CrCasRenterContractStatistic.FindAll(x => company == x.CrCasRenterContractStatisticsLessor).DistinctBy(x => x.CrCasRenterContractStatisticsMembership).Count();
            var profession_count = _unitOfWork.CrCasRenterContractStatistic.FindAll(x => company == x.CrCasRenterContractStatisticsLessor).DistinctBy(x => x.CrCasRenterContractStatisticsJobs).Count();
            var Rigon_count = _unitOfWork.CrCasRenterContractStatistic.FindAll(x => company == x.CrCasRenterContractStatisticsLessor).DistinctBy(x => x.CrCasRenterContractStatisticsRenterRegions).Count();
            var City_count = _unitOfWork.CrCasRenterContractStatistic.FindAll(x => company == x.CrCasRenterContractStatisticsLessor).DistinctBy(x => x.CrCasRenterContractStatisticsRenterCity).Count();
            var Age_count = _unitOfWork.CrCasRenterContractStatistic.FindAll(x => company == x.CrCasRenterContractStatisticsLessor).DistinctBy(x => x.CrCasRenterContractStatisticsAgeNo).Count();
        ...
        public async Task<IActionResult> GetAllByType(string Type, string listDrop, string singleNo, string startDate, string endDate, string company_now)
        {
            ...
            if (Type == "Nationality")
            {
                var AllNationalities = _unitOfWork.CrCasRenterContractStatistic.FindAll(x => company == x.CrCasRenterContractStatisticsLessor, new[] { "CrCasRenterContractStatisticsNationalitiesNavigation" }).Where(x => x.CrCasRenterContractStatisticsNationalitiesNavigation?.CrMasSupRenterNationalitiesStatus != Status.Deleted).DistinctBy(x => x.CrCasRenterContractStatisticsNationalities).ToList();
            ...
            if (Type == "MemperShip")
            {
                var AllCarContracts2_MemperShip = _unitOfWork.CrCasRenterContractStatistic.FindAll(x => company == x.CrCasRenterContractStatisticsLessor, new[] { "CrCasRenterContractStatisticsMembershipNavigation" }).Where(x => x.CrCasRenterContractStatisticsMembershipNavigation?.CrMasSupRenterMembershipStatus != Status.Deleted).DistinctBy(x => x.CrCasRenterContractStatisticsMembership).ToList();
            ...
            if (Type == "Age")
            {
                var AllRenters2_Age = _unitOfWork.CrCasRenterContractStatistic.FindAll(x => company == x.CrCasRenterContractStatisticsLessor).DistinctBy(x => x.CrCasRenterContractStatisticsAgeNo).ToList();
